...
@Component({
  selector: 'app-nota-ap3',
  templateUrl: './nota-ap3.component.html',
  styleUrls: ['../../shared/notas.css'],
  moduleId: module.id,
})
export class NotaAP3Component {

  constructor(private calculadoraServico: CalculadoraService) { }
  ...
  validarNota() {
    if (
      ...
      (this.calculadoraServico.notaAP2 === undefined || this.calculadoraServico.notaAP2 === '')
    ) {
      alert({
        title: 'Campos vazios',
      ...
      (this.calculadoraServico.notaAP2 >= 0) &&
      (this.calculadoraServico.notaAD1 <= 10) &&
      (this.calculadoraServico.notaAP1 <= 10) &&
      (this.calculadoraServico.notaAD2 <= 10) &&
      (this.calculadoraServico.notaAP2 <= 10)
    ) {
      this.calculadoraServico.calculaNota(this.pesoAD, 'AP3');
    } else {
      alert({
